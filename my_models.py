...
class ProteinCNN(nn.Module):
    ...
    def forward(self, v):
        ...
        print("p1.size: ", v.size())
        v = v.transpose(2, 1)
        print("p2.size: ", v.size())
    ...
    def prot_data_aug(self, prot_data):

        b, i, j = prot_data.size()
        prot_data = prot_data.reshape(b, i*j)
        mat_mask = torch.zeros((b, i*j), dtype=torch.float32)
        ...
        _prot_data = torch.zeros(prot_data.size(), dtype=torch.int64)
        ...
        return _prot_data,  mat_mask

class CoAttentionLayer(nn.Module):
    def __init__(self, v_dim, q_dim, h_dim, h_out):
        super(CoAttentionLayer, self).__init__()
        self.Wv = nn.Linear(v_dim, h_dim)
        self.Wq = nn.Linear(q_dim, h_dim)
        self.Wo = nn.Linear(h_dim, h_out)

    def forward(self, v, q):
        # Co-attention mechanism
        v_proj = self.Wv(v)
        q_proj = self.Wq(q)
        att_weights = torch.bmm(v_proj, q_proj.transpose(1, 2))
        att_weights = F.softmax(att_weights, dim=-1)

        # Context vectors
        context_v = torch.bmm(att_weights, q_proj)
        context_q = torch.bmm(att_weights.transpose(1, 2), v_proj)

        return context_v, att_weights
